<#
    .SYNOPSIS
    TA-RoundingUtilsUSB Setup Wizard
    Flashes CircuitPython, installs HID library, and deploys MTCP payload.
#>
param (
...
function Show-Header {
    ...
    Write-Host "     TA-RoundingUtilsUSB  |  Setup Wizard    " -ForegroundColor Cyan
    Write-Host "  ============================================" -ForegroundColor DarkCyan
    Write-Host ""
}
...
if ($installMode -in @("FULL","UPDATE")) {
    ...
    $tmp = Join-Path $env:TEMP "mtcp_$([System.IO.Path]::GetRandomFileName().Replace('.',''))"
